...
, NAME   VARCHAR2(10)
, ADDR   VARCHAR2(20)
)TABLESPACE TBS_EDUA;            -- 테이블스페이스 TBS_EDUA로 지정
--==>> Table TBL_EXAMPLE2이(가) 생성되었습니다.

--○ TBL_EXAMPLE1 과 TBL_EXAPLE2 테이블이
--  각각 어느 테이블스페이스에 저장되어 있는지 조회
...
BONUS	        USERS
SALGRADE	    USERS
TBL_EXAMPLE1	USERS            두 테이블의 저장위치가 다른걸
TBL_EXAMPLE2	TBS_EDUA         확인 가능함.
*/
...
DESCRIBE DEPT;
/*
이름     널?       유형           널? → 비워둬도 되냐 ? / NOT NULL → 안됨 / EX) 회원가입 필수 입력정보
------ -------- ------------ 
...
/*
 cf) MSSQL 서버의 정수 표현 타입
     ...
     MSSQL 서버의 문자 표현 타입
      char, varcchar, Nvarchar


※ ORACLE 은 숫자 표현 타입이 한 가지로 통일되어 있ㅎ다.
    ...
    2. 문자형 CHAR         → 고정형 크기
              ...
              CHAR(10)     ← '곧잔다김민성' 원래 12Byte > 10Byte 초과 → 에러발생!!!
              
                
              VARCHAR2     → 가변형 크기
              VARCHAR2(10) → 담긴 데이터에 따라 크기가 변화
              VARCHAR2(10) ← '김정용'         글씨만 6Byte, 10Byte로 지정했지만 6Byte로 변환!!!
              VARCHAR2(10) ← '멀다김정용'     글씨만 10Byte, 길이 10Byte 문제 없음
              VARCHAR2(10) ← '가까이김정용'   글씨만 12Byte, 10Byte를 초과해서 오류 발생!!!!!!
              
              NCHAR        → 유니코드 기반 고정형 크기
                              -------------
                                 글자수 
                                 
              NCHAR(10)    → 10글자 담을수 있음               
...
JOB               VARCHAR2(9)  
MGR               NUMBER(4)    
HIREDATE          DATE     ←   HIREDATE 는 DATE 타입~     
SAL               NUMBER(7,2)  
COMM              NUMBER(7,2)  
DEPTNO            NUMBER(2)  
*/
...
--○ EMP 테이블에서 부서번호가 20번인 직원들의 데이터들중
--   사원번호, 사원명, 직종명, 급여, 부서번호 항목을 조회한다.

SELECT empno, ENAME, JOB, SAL, DEPTNO
FROM EMP                    
WHERE DEPTNO = 20;       -- 사원번호가 20 인 곳에서 ※ JAVA처럼 == 아님!
/*
...
SELECT EMPNO AS "사원번호", ENAME "사원명", JOB 직종명, SAL "급    여", DEPTNO "부서번호"    --컬럼의 별칭 = 컬럼 이름 AS "별칭"  , 이때 AS, "" 생략 가능
FROM EMP;                                               --  큰따옴표없이 공백 삽입 불가 
