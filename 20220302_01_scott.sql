...
-- 제품 코드와 주문 수량 값이 같은 행만 추출하고자 한다.
...
SELECT T1.JUNO "주문번호", T1.JECODE "제품코드", T1.JUSU "주문량", T1.JUDAY "주문일자"
FROM 
(
...
98775	꼬북칩	30	2022-03-02 09:34:58
98776	모옹쉘	30	2022-03-02 11:28:18
98777	모옹쉘	10	2022-03-02 14:08:12
98778	빼빼로	40	2022-03-02 14:08:41
98779	맛동산	20	2022-03-02 14:09:13
98780	모옹쉘	20	2022-03-02 14:13:41
...
--------------------------------------------------------------------------------
--※ TBL_JUMUN_scott.sql 파일을 통해 데이터 갱신함.
--------------------------------------------------------------------------------
...
/*
   ...
   19	빼빼로	20	2001-11-19 15:16:17    ◀ 
   20	빼빼로	30	2001-11-20 16:17:18    ◀
98780	모옹쉘	20	2022-03-02 14:13:43      나오면 안됨...
98781	빼빼로	30	2022-03-02 14:13:47    ◀ 
98765	빼빼로	20	2022-03-02 09:30:30    ◀
98776	모옹쉘	30	2022-03-02 11:28:12    ◀ 
*/
...
--방법 2 는 겹치는걸 찾는게 아니라
-- 몽쉘 빼빼로 20 30 으로 만든 경우의 수가 나오게 된거다
...
98765	빼빼로	20	2022-03-02 09:30:30
98776	모옹쉘	30	2022-03-02 11:28:12
근데 이건 우리가 결과를 알기 때문에 이렇게 작성할 수 있었지, 모르면 어떻게 작성할껀가..
*/
-- 방법 2 최종
SELECT *
FROM 
...
--   사원번호, 사원명, 직종명, 급여항목을 조회하는 쿼리문을 구성한다.

SELECT EMPNO "사원번호", ENAME "사원명", JOB "직종명", NVL(SAL,0) + NVL(COMM,0)"급여"
FROM TBL_EMP
WHERE NVL(SAL,0) + NVL(COMM,0) =
                               ( SELECT T.급여
                                 FROM
                                 (
                                    SELECT NVL(SAL,0) + NVL(COMM,0)"급여", RANK() OVER(ORDER BY NVL(SAL,0) + NVL(COMM,0) DESC) "랭크"
                                    FROM TBL_EMP
                                 )T
                                 WHERE T.랭크 =1
                               );
...
SELECT EMPNO, ENAME, JOB, SAL
FROM TBL_EMP
WHERE SAL = ALL(800, 1600, 1250, 2975, 1250, 2850, 2450, 3000, 5000, 1500, 950, 3000, 1300, 1500);
--==>> 조회결과 없음 (위에 숫자와 급여가 같은 사람 찾아라 )

SELECT EMPNO, ENAME, JOB, SAL
FROM TBL_EMP
WHERE SAL >= ALL(800, 1600, 1250, 2975, 1250, 2850, 2450, 3000, 5000, 1500, 950, 3000, 1300, 1500);
--==>> 7839	KING	PRESIDENT	5000 (위에 숫자보다  급여가 큰 사람 찾아라 )


--==>> 7839	KING	PRESIDENT	5000 

--○ TBL_EMP 테이블에서 20번 부서에 근무하는 사원중
--   급여가 가장 많은 사원의
...
--○ TBL_EMP 테이블에서 수당(COMM)이 가장 많은 사원의
...
SELECT EMPNO, ENAME, DEPTNO, JOB, COMM
FROM TBL_EMP
WHERE COMM = ALL(SELECT MAX(COMM)    -- MAX 는 NULL 제외.. 
                  FROM TBL_EMP);
--==>> 7654	MARTIN	30	SALESMAN	1400
...
-- 이러면 중복된 값을 또 찾는 행위를 한다.. 7698 5번 찾는다..
...
*/      
-------------------------------------------------------------------------------
--※ JOIN 추가 → NATURAL JOIN (JOIN 적은곳에 추가로 적을것 !)
...
1018	신시은	    9910322234567	2022-02-23	5000
*/
...
--○ 데이터 수정
-- 실수로 TBL_SAWON의 모든 SANAME 을 똘똘이로 바꿔버렸다...
...
SET SANAME = (SELECT SANAME
              ...
              WHERE SANO = TBL_SAWON.SANO)    -- UPDATE 구문에서 이렇게 작성 가능하다.
WHERE SANAME = '똘똘이';
