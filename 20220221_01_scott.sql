...
--   하지만, 맨 위 커리문①(OR)이 가장 빠르게 처리된다.
--   물론 메모리에 대한 내용이 아니라 CPU 처리에 대한 내용이므로
--   이 부분까지 감안하여 쿼리문을 구성하게 되는 경우는 많지 않다.
--   → ②,③에서 『IN』과『=ANY』는 같은 연산자 효과를 가진다. 
--     이들 모두는 내부적으로 『OR』구조로 변경되어 연산 처리 된다.
-------------------------------------------------------------------------------
...
INSERT INTO TBL_SAWON(SANO, SANAME, JUBUN, HIREDATE, SAL)
VALUES(1006, '이현이', '8005122234567', TO_DATE('1999-10-10', 'YYYY-MM-DD'), 1000);
...
1006	이현이	    8005122234567	1999-10-10	1000
...
SELECT *
FROM TBL_SAWON
WHERE SANAME ='김';    -- 이는 이름이 '김' 인 사람만 찾는것 김이 포함된 사람 찾는게 아님
--==>> 조회 결과 없음

SELECT *
FROM TBL_SAWON
WHERE SANAME LIKE '김__';   -- 김 + 언더바 2개 → 김ㅇㅇ 이름 찾음
--==>> 1001	김민성	9707251234567	2005-01-03	3000

SELECT *
FROM TBL_SAWON
WHERE SANAME LIKE '김%';-- 김 + % 김으로 시작하는 사람 찾기.
--==>> 1001	김민성	9707251234567	2005-01-03	3000
...
이현이	8005122234567	1000
*/

--○ TBL_SAWON 테이블에서 이름의 마지막 글짜가 『민』인 사원의
--   사원명, 주민번호, 급여 항목을 조회한다.
SELECT SANAME, JUBUN, SAL
FROM TBL_SAWON
WHERE SANAME LIKE '%민';
...
SELECT SANAME, JUBUN, SAL
FROM TBL_SAWON
WHERE SANAME LIKE '%이' OR SANAME LIKE '이%' OR SANAME LIKE '_이_';
/*
이지연	9905192234567	3000
이연주	9508162234567	4000
오이삭	9805161234567	4000
이현이	8005122234567	1000
박한이	0204053234567	1000
...
SELECT SANO, SANAME, SAL
FROM TBL_SAWON
WHERE SANAME LIKE '선%' AND SANAME NOT LIKE '선우%';
--==>> 1008	선동렬	1500
...
--※ ESCAPE 로 정한 문자의 다음 한 글자를 와일드카드에서 탈출 시켜라...
...
40	OPERATIONS	BOSTON
*/
--> 『50 개발부 서울』데이터가 소실되었음을 확인(존재하지 않음)
...
--   DML 구문을 사용한 후 COMMIT 을 하고나서 ROLLBACK을 실행해봐야
--   아무런 소용이 없다.
...
-- 1. 조회부터 하기 (내가 지울걸 정확히 확인하자!!!!)
SELECT *
FROM TBL_DEPT
WHERE DEPTNO = 50;

--2. SELECT * 을 DELETE 로 바꾸기 
DELETE 
FROM TBL_DEPT
WHERE DEPTNO = 50;
--==>> 1 행 이(가) 삭제되었습니다.
...
SELECT ENAME "사원명", DEPTNO "부서번호",
        JOB "직종명", SAL "급여", SAL*12+NVL(COMM,0) "연봉"
FROM EMP;
/*
...
SELECT ENAME "사원명", DEPTNO "부서번호",
        JOB "직종명", SAL "급여", SAL*12+NVL(COMM,0) "연봉"
FROM EMP
ORDER BY DEPTNO;           -- ASC 를 명시하지 않아도 오름차순으로 정렬
/*
CLARK	10	MANAGER	    2450	29400
KING	10	PRESIDENT	5000	60000
MILLER	10	CLERK	    1300	15600
...
SELECT ENAME "사원명", DEPTNO "부서번호",
        JOB "직종명", SAL "급여", SAL*12+NVL(COMM,0) "연봉"
FROM EMP
ORDER BY DEPTNO DESC;      -- 정렬기준 : DESC 내림차순(생략 불가!!)
/*                         -- DESCRIBE 와 DESC 로쓰는 건 같지만 문맥에 맞게 오라클이 알아서 판단.                                         
BLAKE	30	MANAGER	    2850	34200
TURNER	30	SALESMAN	1500	18000
ALLEN	30	SALESMAN	1600	19500
MARTIN	30	SALESMAN	1250	16400
...
SELECT ENAME "사원명", DEPTNO "부서번호",
        JOB "직종명", SAL "급여", SAL*12+NVL(COMM,0) "연봉"
FROM EMP
ORDER BY 연봉 DESC;       -- 위에서 별칭을 연봉 만들었기 때문에 작성 가능 
/*                        -- FROM, WHERE, GROUP BY, HAVING, SELECT, ORDER BY 순이기 때문에
KING	10	PRESIDENT	5000	60000   -- SELECT에서 만든 별칭으로 ORDER BY 에서 사용이 가능하다.
FORD	20	ANALYST	    3000	36000
SCOTT	20	ANALYST	    3000	36000
JONES	20	MANAGER	    2975	35700
BLAKE	30	MANAGER	    2850	34200
...
SELECT ENAME "사원명", DEPTNO "부서번호",
        JOB "직종명", SAL "급여", SAL*12+NVL(COMM,0) "연봉"
FROM EMP
ORDER BY 2;   --부서번호 오름차순 , 숫자는 SELECT에서 선언한 순서 이다.(1부터 시작...)
-- ORDER BY 부서번호; --(같은 구문임) 
--> EMP 테이블이 가지고 있는 테이블의 고유한 컬럼순서 (2→ ENAME, 사원명)가 아니라
--  SELECT 처리되는 두 번째 컬럼(2→ DEPTNO, 부서번호)을 기준으로 정렬되는 것을 확인.
--  ASC 생략된 상태 → 오름차순으로 정렬되는 것을 확인
--  즉 이 구문에서 『ORDER BY 2;』는『ORDER BY DEPTNO ASC;』 와 같은 구문이다.
/*
CLARK	10	MANAGER	    2450	29400
KING	10	PRESIDENT	5000	60000
MILLER	10	CLERK	    1300	15600
...
FROM EMP
ORDER BY 2, 4; -- DEPTNO로 먼저 정렬, 그 후 안에서 SAL 기준... ASC
/*
...
FROM EMP
ORDER BY 2, 3, 4 DESC;
--> ① 2 → DEPT(부서번호) 기준 오름차순 정렬
--> ② 3 → JOB(직종명)    기준 오름차순 정렬
--> ③ 4 → SAL(급여) 기준 『내림차순』 정렬 → 3번째 수행이라 직종이 겹치는 곳에서 내림차순 확인 가능
/*
MILLER	10	CLERK	1300
CLARK	10	MANAGER	2450
KING	10	PRESIDENT	5000
...
FROM EMP; -- COL1과 COL2 가 같은 기능을 수행함.
/*
...
--  내부적으로 숫자나 날짜를 문자로 바꿔주는 과정이 포함되어 있다.
...
SELECT ENAME "COL1"
     , SUBSTR(ENAME, 1, 2) "COL2" -- ENAME 에서 1번째 글자부터 총 2개를 따와라...
FROM EMP;
--> 문자열을 추출하는 기능을 가진 함수
--  첫 번째 파라미터 값은 대상 문자열(추출의 대상, TARGET)
--  두 번째 파라미터 값은 추출을 시작하는 위치(인덱스는 1 부터 시작)
--  세 번째 파라미터 값은 추출할 문자열의 갯수(생략시 끝까지..)
/*
SMITH	SM
ALLEN	AL
WARD	WA
...
--  두 번째 파라미터 값에 해당하는 문자열이 등장하는 위치를 찾아라...
--  세 번째 파라미터 값은 찾기 시작하는 위치(스캔하는 위치)
--  네 번째 파라미터 값은 몇 번째 등장하는 값을 찾을 것인지에 대한 설정(중복)
--  즉,COL2는 ORACLE ORAHOME BIORA를 처음부터 스캔해서 첫 ORA를 찾은 위치가 어디인가!!
--  COL3은 ORACLE ORAHOME BIORA를 처음부터 스캔해서 두 번째 ORA를 찾은 위치가 어디인가!! → 8에 위치...
--  COL4는 2에서 시작해서 첫 ORA에서 R부터 시작하기 때문에 첫 ORA를 못찾음 그래서 COL3과 같이 8에 위치.
--  COL5는 4번째 파라미터를 생략한 것으로 2에서 시작해서 ORA를 찾는구문 . COL4와 같은 문장.. → 8에 위치
--  COL6은 2부터 시작해서 그 뒤는 ORA가 2개 있기 때문에 3번째 ORA는 없다 → 0 나옴...
--  COL7은 뒤에서 3번째 부터 왼쪽으로 스캔해서 첫 ORA 를 찾음  → 18번 째 
--  COL8 COL9는 뒤에서 4번째 부터 시작해서 각각 첫 ORA 두번째 ORA를 찾음...
...
FROM TBL_FILES;       -- C:\AAA\BBB\CCC\SALES.DOC 를 뒤에부터 시작해서 \ 찾고 거기위치+1(\)까지 짤라버림.
/*
1	SALES.DOC
2	PANMAE.XXLS
3	RESEARCH.PPT
...
--                        COD.SELAS\CCC\BBB\AAA\:C 에서 처음부터 첫 역슬래쉬까지-1 까지 짜르고 다시 REVERSE
FROM TBL_FILES;
/*
1	SALES.DOC
2	PANMAE.XXLS
...
FROM DUAL; --길이 24 , 마지막 '\' 위치 15번째 

SELECT INSTR(REVERSE('C:\AAA\BBB\CCC\SALES.DOC'), '\',1)
FROM DUAL; -- 뒤에서 10 번째  

-- 마지막 '\' 위치  24 에서 뒤에서 10번째
-- 24 - 10  = 14 + 1  = 15 번째 

-- LENGTH(FILENAME) - REVERSE(FILENAME, '\', 1) + 1  =  마지막 '\' 위치 
...
SELECT 'ORAORAORACLEROLACLE' "COL1"
    ...
    ,  LTRIM('AAAAAAAAAAAAAAAAAORAORAORACLEORACLE', 'ORA') "COL3" -- 'O''A''R' AAAAAAA도 짤림 'ORA'이렇게 짜르는게 아님
    ,  LTRIM('ORAoRAORACLEORACLE', 'ORA') "COL4"        -- 대소문자 구분 함!
    ,  LTRIM('ORAORA ORACLEORACLE', 'ORA') "COL5" -- 공백에서 끊김
    ,  LTRIM('        ORACLE' ,' ') "COL6"     -- 공백을 짜를수 있음
    ,  LTRIM('        ORACLE') "COL7"         -- 공백을 짜를 때는 두 번째 파라미터 스킵 가능
FROM DUAL;
/*
ORAORAORACLEROLACLE	
CLEORACLE	
...
--  같은 글자가 등장할 경우 이를 제거한 걸과값을 반환한다.
--  단, 완형형으로 처리되진 않음.
...
SELECT RTRIM('이김신박이김신이김김신신신이김김김이신김신이이김신', '이김신') "RESULT"
FROM DUAL;
--==>> 이김신박

SELECT RTRIM('이김                               ') "RESULT"
FROM DUAL;
--==>> 이김
